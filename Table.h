...
class Table {
 ...
 private:
   ...
   // Dynamic array to work as a hash table of user specified size during run-time
   // Size doesn't change once dynamic array is created 
   ListType * hashTable;
   
   // Fixed-size array of size HASH_SIZE (9973) created at compile time. 
   ListType  hashTableFixed[HASH_SIZE];
   
   // Variable indicating number of name-score entries in hash table
   int numOfEntries;
   
   // Method to return number of non-empty buckets in hash table
   int nonEmptyBuckets() const;
   
   // Method to return length of longest chain in hash table
   int longestChainSize() const;
